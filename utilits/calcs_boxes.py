    ...
    def calc_boxes(self, img_path, model_yolov5, config):
        ...
        if config.is_draw:
            ...
            if not os.path.isdir(os.path.join(config.path_to_predicted_images, self.folder_name)):
                os.mkdir(os.path.join(config.path_to_predicted_images, self.folder_name))
...
#     ...
#     """
#     def __init__(self, model_yolov5, config: dict):
#         super().__init__()
#         self.config = config
#         #self.folder_name = os.path.basename(os.path.dirname(img_path))
#         #self.model_yolov5 = model_yolov5
#         #self.img_path = img_path
#         #self.num_boxes = self.calc_boxes(img_path, model_yolov5)
    
#     def calc_avg_num_blastos(self, path_to_imgs, folder_name, model, is_draw):
